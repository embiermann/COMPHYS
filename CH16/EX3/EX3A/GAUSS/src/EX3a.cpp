...
int main (int argc, char * * argv) {
  ...
  // Gaussian Fit
  Parameter pMu("Mu", 0.5, 0.1, 4.0);
  Parameter pSig("Sig", 0.5, 0.1, 4.0);

  Sqrt sqrt;
  Exp exp;
  Variable X;
  GENFUNCTION gauss = 1./(sqrt(2.*M_PI)*pSig)*exp(-(X-pMu)*(X-pMu)
						  /(2.*pSig*pSig));
  ...
  double wavelength = minimizer.getValue(&pMu);
  std::cout<<"E = "<<h*c/wavelength<<" eV"<<std::endl;

  // Print Experimental Resolution
  std::cout<<"sigma = "<<minimizer.getValue(&pSig)<<std::endl;

  // Covariance and Chi^2

  Eigen::MatrixXd C(2,2);
  C(0,0)=minimizer.getError(&pMu,&pMu);
  C(1,1)=minimizer.getError(&pSig,&pSig);
  C(0,1)=minimizer.getError(&pMu,&pSig);
  C(1,0)=minimizer.getError(&pSig,&pMu);
