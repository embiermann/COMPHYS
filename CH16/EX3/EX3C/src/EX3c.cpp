...
int main (int argc, char * * argv) {
  ...
  Exp exp;
  Variable X;
  GENFUNCTION dist = 1./(sqrt(2.*M_PI)*pSig1)*exp(-(X-pMu1)*(X-pMu1)
						  /(2.*pSig1*pSig1))
  ...
  // Print Experimental Smear for Each
  double gam1 = minimizer.getValue(&pGam1);
  double gam2 = minimizer.getValue(&pGam2);
  double sig1 = minimizer.getValue(&pSig1);
  double sig2 = minimizer.getValue(&pSig1);
  std::cout<<"Smear of peak 1 = "<<gam1-sig1<<std::endl;
  std::cout<<"Smear of peak 2 = "<<gam2-sig2<<std::endl;
